...
TEST(Test_CounterLimit_Init1) {
  ...
  Counter base;
  Counter_InitLimits(&base, lower_limit, upper_limit);

  ASSERT(Counter_GetValue(&base) == expected,
...
TEST(Test_CounterLimit_Count1) {
  ...
  CountHelper(&base, expected + (upper_limit + 1), step);

  ASSERT_INT(Counter_GetValue(&base), expected);
}
...
TEST(Test_CounterLimit_Count2) {
  ...
  CountHelper(&base, expected + (3 * (upper_limit + 1)), step);
...
TEST(Test_CounterLimit_Count3) {
  ...
  ASSERT_INT(Counter_GetValue(&base), upper_limit - expected + 1);
}
...
TEST(Test_CounterLimit_Count4) {
  ...
  CountHelper(&base, expected + (5 * (upper_limit + 1)), step);
...
TEST(Test_CounterLimit_Reset1) {
  ...
  CountHelper(&base, 4, step);

  Counter_Reset(&base);

  ASSERT(Counter_GetValue(&base) == expected, "Counter reset failed");
}
